...
const defaultFormState: ManufacturingItemForm = {
  groupType: 'Manufacturing Item',
  code: '',
  itemGroup: '',
  description: '',
  specification: '',
...
export default function ManufacturingItemScreen() {
  const { toast } = useToast();
  const [viewMode, setViewMode] = useState<'FORM' | 'LIST'>('LIST');
  const [rows, setRows] = useState<ManufacturingItemForm[]>([]);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        ...
        toast('Manufacturing item created successfully.');
      }
      setViewMode('LIST');
      loadItems();
    } catch (err) {
  ...
  const filteredRows = rows.filter(r => {
    ...
    return (
      ...
      (r.drawingNumber && r.drawingNumber.toLowerCase().includes(s))
    );
  });
    ...
    <>
      ...
      {viewMode === 'LIST' ? (
        <div className="panel">
          <div className="panel-h">
            <h2><span className="material-symbols-rounded">inventory_2</span> Manufacturing Items ({filteredRows.length})</h2>
            <input
              type="text"
              className="in"
              placeholder="Search by Code, Item Name, Drawing..."
              value={search}
      ...
      {deleteTarget && (
        <ConfirmActionModal
          ...
          body={`Are you sure you want to delete manufacturing item "${deleteTarget.code}"?`}
          okLabel="Delete"
          onConfirm={del}
