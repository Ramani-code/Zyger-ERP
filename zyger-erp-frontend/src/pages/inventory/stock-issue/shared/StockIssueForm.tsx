...
import {
  ...
  useStockIssueDocument,
  useStockIssueLookups,
  useStockIssueMutations,
  useStockIssueNextNumber,
...
export default function StockIssueForm({
  ...
  const lookups = useStockIssueLookups();
  const approvedRequestsQuery = useApprovedIssueRequests();
  const documentQuery = useStockIssueDocument(config, documentId ?? null);
  const nextNumberQuery = useStockIssueNextNumber(config);

  const { createMutation, updateMutation, actionMutation } =
    useStockIssueMutations(config);

  const [form, setForm] = useState<StockIssueFormState>(() =>
    createEmptyForm(config)
  );
  const [currentDocument, setCurrentDocument] =
    useState<StockIssueDto | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const results = await stockIssueService.checkAvailability(
          availabilityPairs,
          controller.signal
        );
  ...
  const updateBase = (
    key: 'date' | 'sourceLocation' | 'remarks',
    value: string
  ) => {
    setForm((previous) => {
      const next = {
      ...
      .then((sir) => {
        ...
        setForm((previous) => {
          ...
          const fieldMap: Record<string, string | undefined> = {
            department: sir.department,
            purpose: sir.purpose,
            jobOrderNo: sir.jobOrderNo,
          };
  ...
  const updateLine = (
    index: number,
    key: keyof StockIssueLineFormState,
    value: string
  ) => {
    setForm((previous) => {
  ...
  const resolveOptions = (field: StockIssueHeaderFieldConfig) => {
  ...
  const renderCustomField = (field: StockIssueHeaderFieldConfig) => {
    const value = form.fields[field.key] ?? '';

    if (field.fixed) {
      return <input className="in" value={field.fixed} readOnly tabIndex={-1} />;
    }
  ...
  const getAvailableDisplay = (line: StockIssueLineFormState): string => {
    const effectiveLocation = line.location || form.sourceLocation;
          ...
          <div className="fgrid">
            ...
            <label className="fld">
              ...
              <input
                ...
                value={form.date}
                readOnly={!editable}
                onChange={(event) => updateBase('date', event.target.value)}
              />
            </label>
            ...
            <label className="fld">
              ...
              <select
                ...
                disabled={!editable}
                onChange={(event) =>
                  updateBase('sourceLocation', event.target.value)
                }
              >
                <option value="">— Select —</option>
            ...
            <label className="fld span2">
              ...
              <input
                ...
                value={form.remarks}
                readOnly={!editable}
                onChange={(event) => updateBase('remarks', event.target.value)}
              />
            </label>
          </div>
        </div>
